...
const URLS: { path: string; screen: string }[] = [
  ...
  { path: '/42/players', screen: leagueTabTitles.players },
  { path: '/42/market', screen: leagueTabTitles.market },
  { path: '/42/league', screen: leagueTabTitles.league },
  { path: '/42/more', screen: leagueTabTitles.more },
...
describe('Route-Baum', () => {
  it.each(URLS)('$path zeigt „$screen"', async ({ path, screen: name }) => {
    renderRoute(path);
    // Über die Überschrift statt über einen Testid: jeder Screen rendert
    // AppHeader mit seinem Titel, die Assertion überlebt also den Port.
